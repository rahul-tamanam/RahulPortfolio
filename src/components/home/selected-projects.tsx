...
function Card(props: CardProps) {
  ...
  return (
    <Link key={slug} href={`/projects/${slug}`} className='group block'>
      <BorderRotate
        className='relative rounded-2xl shadow-feature-card'
        animationMode='rotate-on-hover'
        animationSpeed={3}
        gradientColors={{
          primary: '#064e3b', // deep teal-green
        ...
        borderWidth={1.5}
      >
        <div className='rounded-[inherit] bg-card p-2'>
          ...
          <div className='relative overflow-hidden rounded-lg'>
            <BlurImage
              ...
              imageClassName='rounded-lg object-cover object-[50%_60%]'
              lazy={false}
              fetchPriority='high'
            />
            ...
            {/* Hover overlay with "Go to project" */}
            <div className='pointer-events-none absolute inset-0 flex items-center justify-center opacity-0 backdrop-blur-xl transition-opacity duration-200 group-hover:opacity-100'>
              <div className='rounded-xl bg-background/60 px-4 py-2 text-sm font-medium text-foreground shadow-lg sm:text-base'>
                Go to project
              </div>
            </div>
          </div>
